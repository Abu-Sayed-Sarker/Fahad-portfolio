    ...
    <footer className="w-full py-16 px-6 flex flex-col items-center gap-12 relative overflow-hidden bg-primary">
      ...
      <nav className="flex flex-wrap justify-center gap-x-10 gap-y-4 relative z-10">
        {navLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
            className={`text-[#c8cee8]/80 text-sm tracking-wide transition-all duration-300 hover:text-white hover:drop-shadow-[0_0_8px_rgba(124,58,237,0.5)] ${link.name === "Contact me"
      ...
      {/* <div className="flex items-center gap-6 relative z-10">
        {[
          ...
          { icon: <Linkedin size={18} />, href: "https://www.linkedin.com/in/istiaq-ahmmed-fahad/" },
          { icon: <Github size={18} />, href: "https://github.com/iaf12" },
        ].map(({ icon, href }, idx) => (
          <a
            key={idx}
            href={href}
      ...
      <div className="text-center relative z-10">
        <p className="text-[#5c5c7b] text-xs">
          © 2025 Istiaq Ahmmed Fahad. All rights reserved.
        </p>
